			...
			switch (cursors[i].state) {
				case CursorState.added:
					/* let cursor: Entity = this.world.registerEntity(new Entity())
						.registerComponent(new ShapeRenderComponent(new Circle(20), new Color(255, 255, 255)))
						.entity;
					cursor.transform.origin = new Vector2D(cursors[i].x, cursors[i].y); */
				...
				case CursorState.moved:
					if (cursors[i].data instanceof Camera) {
						...
						if (cursors[i].wheelY !== 0) {
							...
							//	let force: ForcePhysicsComponent = (cursors[i].data as Entity)
							//.getComponent<ForcePhysicsComponent>(ForcePhysicsComponent);
